...
void	print_distribution(t_solution const *s)
{
	int		i;

	printf("Distribution (%d turns) [\n", s->n_turns);
	i = 0;
	while (i < s->n_paths)
	{
		printf("  %d ant(s) to path %s... of length %d\n", s->numbers[i], s->paths[i].origin->name, s->paths[i].length);
		++i;
	}
	printf("]\n");
}
...
int		main(void)
{
	...
	while (bhandari(&g))
	{
		...
		printf("\n%d path(s) found:\n", n_paths);
		print_paths(&g);			//
		s = build_solution(n_ants, g.start, n_paths);
		print_distribution(s);		//
		print_solution(s, n_ants);
		solution_destroy(s);
	}
